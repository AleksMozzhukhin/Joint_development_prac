...
async def broadcast_message(message, exclude_writer=None, format_args=None):
    """
    ...
    Args:
        message: Сообщение для отправки (может содержать {} для форматирования).
        exclude_writer: Клиент, которому не нужно отправлять сообщение.
        format_args: Словарь с аргументами для форматирования сообщения.
    """
    to_remove = []
    ...
    for writer, username in clients.items():
        if writer != exclude_writer:
            try:
                ...
                print(f"DEBUG: user='{username}', message='{message}', translated_message='{translated_message}'")
                # Применяем форматирование, если есть аргументы
                if format_args:
                    for key, value in format_args.items():
                        try:
                            tr = translator.gettext(value)
                            format_args[key] = tr
                        except Exception:
                            pass
                    translated_message = translated_message.format(**format_args)
...
async def move_monsters():
    ...
    while True:
        ...
        while not success and attempts < max_attempts:
            ...
            # try:
            #     locale = client_locales.get(clients.items()[0], 'en')
            #     translator = get_translator(locale)
            #
            #     # Переводим направление
            #     translated_direction = translator.gettext(direction_str)
            # Отправляем сообщение всем игрокам
            broadcast_msg = "{const.RESP_BROADCAST}: {monster} moved one cell {direction}"
            broadcast_format_args = {
                'const': const,
                'monster': monster_name,
                'direction': direction_str
            }
            await broadcast_message(broadcast_msg, format_args=broadcast_format_args)
...
class MUDGame:
    ...
    def handle_movemonsters(self, args):
        ...
        if mode == "on":
            ...
            local_msg = "Moving monsters: on"
            broadcast_msg = "{const.RESP_BROADCAST}: User '{username}' turned monster movement ON"
            broadcast_format_args = {
                'const': const,
                'username': self.username
            }
            return local_msg, (broadcast_msg, broadcast_format_args)
        elif mode == "off":
            moving_monsters_enabled = False
            local_msg = "Moving monsters: off"
            broadcast_msg = "{const.RESP_BROADCAST}: User '{username}' turned monster movement OFF"
            broadcast_format_args = {
                'const': const,
                'username': self.username
            }
    ...
    def handle_sayall(self, args):
        ...
        broadcast_msg = "{const.RESP_BROADCAST}: {username}: {message}"
        broadcast_format_args = {
            'const': const,
            'username': self.username,
            'message': message
        ...
        return f"{const.RESP_SAYALL}: Message sent", (broadcast_msg, broadcast_format_args)
    ...
    def handle_addmon(self, args):
        ...
        try:
            ...
            # Сообщение для локализации с параметрами
            broadcast_msg = "{const.RESP_BROADCAST}: User '{username}' added monster '{monster}' with {hp} HP at ({x}, {y})"
            broadcast_format_args = {
                'const': const,
                'username': self.username,
                'monster': monster_name,
            ...
            return result, (broadcast_msg, broadcast_format_args)
        except ValueError:
            return f"{const.RESP_ERROR}: Invalid addmon parameters", None
    ...
    def handle_attack(self, args):
        ...
        try:
            ...
            if monster_hp <= 0:
                ...
                broadcast_msg = "{const.RESP_BROADCAST}: User '{username}' attacked '{monster}' with {weapon}, dealing {damage} HP. {monster} was killed!"
                broadcast_format_args = {
                    'const': const,
                    'username': self.username,
                ...
                broadcast_msg = "{const.RESP_BROADCAST}: User '{username}' attacked '{monster}' with {weapon}, dealing {damage} HP. {monster} has {hp} HP left."
                broadcast_format_args = {
                    'const': const,
                    'username': self.username,
                    'monster': current_monster_name,
...
async def handle_client(reader, writer):
    ...
    try:
        ...
        await broadcast_message(
            "{const.RESP_BROADCAST}: User '{username}' has joined the MUD.",
            exclude_writer=writer,
            format_args={'const': const, 'username': username}
        )
        ...
        while not reader.at_eof():
            ...
            if message.lower() == const.CMD_QUIT or message.lower() == const.CMD_EXIT:
                ...
                if len(parts) == 2:
                    ...
                    if locale_name in translations:
                        ...
                        try:
                            ...
                            # Переводим сообщение
                            translated_message = translator.gettext("Unsupported locale: {}").format(locale_name)
                            writer.write(f"{translated_message}\n".encode())
                        except Exception:
                    ...
                    try:
                        ...
                        # Переводим сообщение
                        translated_message = translator.gettext("Invalid locale format")
                        writer.write(f"{translated_message}\n".encode())
                    except Exception:
                        writer.write("Invalid locale format\n".encode())
                    await writer.drain()
            else:
                response, broadcast_data = games[username].handle_command(message)
                ...
                if broadcast_data:
                    if isinstance(broadcast_data, tuple):
                        # Новый формат с аргументами форматирования
                        broadcast_msg, format_args = broadcast_data
                        await broadcast_message(broadcast_msg, exclude_writer=writer, format_args=format_args)
                    else:
                        # Старый формат для обратной совместимости
                        await broadcast_message(broadcast_data, exclude_writer=writer)
    except Exception as e:
        ...
        if username and username in usernames:
            ...
            await broadcast_message(
                "{const.RESP_BROADCAST}: User '{username}' has left the MUD.",
                exclude_writer=writer,
                format_args={'const': const, 'username': username}
            )
