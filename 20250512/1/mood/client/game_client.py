...
class MUDClient(cmd.Cmd):
    ...
    def do_help(self, arg):
        """
        Показать справку по командам или открыть документацию.

        Использование:
            help                - показать список команд
            help <команда>      - показать справку по конкретной команде
            help all_doc        - открыть полную документацию в браузере

        Args:
            arg: Название команды или 'all_doc' для открытия документации.
        """
        if not arg:
            # Стандартное поведение help без аргументов
            return super().do_help(arg)

        if arg.strip() == "all_doc":
            # Открываем документацию в браузере
            try:
                from ..documentation import open_documentation
                print("Opening full documentation in browser...")
                success = open_documentation()
                if success:
                    print("Documentation opened successfully")
                else:
                    print("Failed to open documentation automatically")
                    print("You can also use 'mood-help' command if package is installed")
            except ImportError:
                print("Documentation module not found")
                print("Documentation may not be included in this installation")
                print("Try installing the full package or use 'mood-help' command")
            except Exception as e:
                print(f"Error opening documentation: {e}")
                print("Try using 'mood-help' command if package is installed")
        else:
            # Стандартное поведение help для конкретных команд
            return super().do_help(arg)
